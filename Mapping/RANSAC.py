...
def find_best_inliers_and_homography(baseImagePoints, newImagePoints, maxIterations=2000, threshold=5.0, confidence=0.8):
    ...
    for element in range(maxIterations):
        ...
        # Project into homogeneous space
        projected_h = (H @ baseImagePointsHomography.T).T  # (N, 3)

        # Safe divide to avoid NaNs
        den = projected_h[:, 2]
        valid = np.abs(den) > 1e-8

        projected = np.zeros((numberOfMatches, 2), dtype=np.float32)
        projected[valid] = projected_h[valid, 0:2] / den[valid, None]
        projected[~valid] = np.inf
...
def run_RANSAC(baseImagePoints, newImagePoints, img):
    ...
    H, inliers = find_best_inliers_and_homography(baseImagePoints, newImagePoints,
                             maxIterations=2000, threshold=5.0, confidence=0.8)
